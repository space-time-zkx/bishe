...
def decorator(model, data, type, save=False,epoch=None):
    '''
    :param model:
    :param data:
    ...
    if type == 'fpointnet':
        ...
        points, seg, center, angle_class, angle_residual, size_class, size_residual,\
                                                            rot_angle, one_hot_vec,box2d,localimage = data
        # print(localimage,"??")
        # print(save,epoch)
        output_dict = model(points, one_hot_vec,box2d,localimage,save=save,epoch=epoch )
        ...
        output_dict = model(patch, one_hot_vec,save=save,epoch=epoch)
        # print(output_dict)
    ...
    # iou3d_correct = 0
    # ac = iou3ds.reshape(iou3ds.shape[0],1) * one_hot_vec.detach().cpu().numpy()
    # for a in ac:
    #     if a[0]>=0.7 or a[1]>=0.5 or a[2]>=0.5:
            # iou3d_correct+=np.sum(a)
