...
class Trainer(object):
    ...
    def train(self):
        for epoch in range(self.cfg['max_epoch']):
            ...
            if (trained_epoch % self.cfg['save_frequency']) == 0:
                os.makedirs('checkpoints_clas', exist_ok=True)
                ckpt_name = os.path.join('checkpoints_clas', 'checkpoint_epoch_%d' % trained_epoch)
    ...
    def train_one_epoch(self,epoch):
        ...
        for batch_idx, batch_data in enumerate(self.train_loader):
            ...
            if batch_idx % 100 == 0 and epoch!=0:
                loss, stat_dict = self.decorator(self.model, batch_data, self.cfg['decorator'],save=True,epoch=epoch)
            elif epoch==0:
                loss, stat_dict = self.decorator(self.model, batch_data, self.cfg['decorator'],save=True,epoch=epoch)
            else:
                loss, stat_dict = self.decorator(self.model, batch_data, self.cfg['decorator'], save=False)
            loss.backward()
            self.optimizer.step()
