...
class FrustumDataset(data.Dataset):
    ...
    def __init__(self, dataset_helper, npoints,
        ...
        if self.from_rgb_detection:
            with open(self.pickle_file, 'rb') as fp:
                ...
                self.imagep_list = pickle.load(fp)
                # print(self.id_list[0],self.box2d_list[0],self.input_list[0],self.type_list[0],self.frustum_angle_list[0],
                # self.prob_list[0],self.imagep_list[0])
        else:
            with open(self.pickle_file, 'rb') as fp:
                self.id_list = pickle.load(fp)
                self.box2d_list = pickle.load(fp)
    ...
    def __getitem__(self, index):
        ...
        # lcimage = np.ones((500,500,3))*114
        localimage = self.imagep_list[index]
        # lcimage[:localimage.shape[0],:localimage.shape[1],:] = localimage
        # print(localimage.shape)
        localimage = torch.from_numpy(localimage)
        localimage = localimage.unsqueeze(0)
        localimage = localimage.transpose(2, 3).transpose(1, 2)
        if localimage.shape[2] > 0 and localimage.shape[3] > 0:
            lcimage = F.interpolate(localimage.float(), [32, 32], mode='bilinear', align_corners=True).squeeze(
                0).numpy()
        else:
            lcimage = np.ones((3, 32, 32))
        ...
        if self.from_rgb_detection:
            ...
            return point_set, rot_angle, self.prob_list[index], self.id_list[index], \
                   self.type_list[index], self.box2d_list[index], one_hot_vec, box2d, lcimage
        ...
        return point_set, seg, box3d_center, angle_class, angle_residual, \
               size_class, size_residual, rot_angle, one_hot_vec, box2d, lcimage

    def get_center_view_rot_angle(self, index):
        ''' Get the frustum rotation angle, it is shifted by pi/2 so that it
        can be directly used to adjust GT heading angle '''
        # print(self.frustum_angle_list[index])
