...
# parser.add_argument('--small_only', action='store_true', help='Only generate frustum data from car instances')
...
#         images = dataset.get_image(data_idx)
#         if args.model == 'pseudo_lidar':
#             pc_lidar = dataset.get_pseudo_lidar(data_idx, 4) # six channels (x, y, z, r, g, b)
...
#                 # print(pc_rect.shape,pc_img.shape)
#                 pc_in_box_fov = pc_rect[box_fov_inds, :]
#                 pc_img_box_fov = pc_img[box_fov_inds,:]
#                 xmin, ymin = max(xmin, 0), max(ymin, 0)   # check range
#                 xmax, ymax = min(xmax, width), min(ymax, heigth)  # check range
#                 imagepatch = np.array(images)[int(ymin):int(ymax), int(xmin):int(xmax)]
#                 # print(pc_in_box_fov.shape,pc_img_box_fov.shape)
...
#                 # print(pc_in_box_fov[:,3])
#                 # print(pc_in_box_fov.shape, pc_img_box_fov.shape)
#                 # print(pc_img_box_fov)
#                 pc_in_box_fov = np.concatenate((pc_in_box_fov,pc_img_box_fov),axis=1)
#                 input_list.append(pc_in_box_fov)
#                 label_list.append(label)
#                 imagep_list.append(imagepatch)
...
#         pickle.dump(imagep_list,fp)
#         # import cv2
#         # cv2.imwrite("test.png",imagep_list[0])
#         # print(len(imagep_list))
#
...
#             calib = dataset.get_calib(data_idx)
#             if args.model == 'pseudo_lidar':
#                 pc_lidar = dataset.get_pseudo_lidar(data_idx, 4)
...
#         xmin, ymin = max(xmin, 0), max(ymin, 0)  # check range
#         xmax, ymax = min(xmax, width), min(ymax, heigth)  # check range
#         imagepatch = np.array(image)[int(ymin):int(ymax), int(xmin):int(xmax)]
...
#     # setting
#     whitelist = ['Car'] if args.small_only else ['Car', 'Pedestrian', 'Cyclist']
#     assert args.model in ['fpointnet', 'pseudo_lidar']
#     output_prefix = args.model
#     output_prefix += '_car_only' if args.small_only else '_carpedcyc'
#
#     train_filename = output_prefix + '_train.pickle'
#     val_filename = output_prefix + '_val.pickle'
#     val_detections_filename = output_prefix + '_msval_rgb_detection.pickle'
#     rgb_detections = os.path.join(ROOT_DIR, 'data/KITTI/2d_detections/fpointnet/rgb_detection_val.txt')
#     # rgb_detections = "/data/zhaokexin/vision/3d_object/mscnn_test.txt"
#     if args.gen_train:
...
parser.add_argument('--gen_val_rgb_detection', action='store_true',
                    help='Generate frustum val split frustum data [from 2d detections]')
parser.add_argument('--model', default='fpointnet', help='fpointnet, pseudo_lidar or am3d')
parser.add_argument('--small_only', action='store_true', help='Only generate frustum data from car instances')
parser.add_argument('--pseudo_lidar', action='store_true', help='Generate frustum data from pseudo lidar')
args = parser.parse_args()
...
def extract_frustum_data_rgb_detection(det_filename,
    ...
    progress_bar = tqdm.tqdm(total=len(det_id_list), leave=True,
                             desc='%s split frustum data gen (from 2d detections)' % split)
    # det_id_list = det_id_list[0:20]
    for det_idx in range(len(det_id_list)):
        # ignore object belong to other categories
        if det_type_list[det_idx] not in whitelist:
        ...
        if cache_id != data_idx:
            calib = dataset.get_calib(data_idx)
            if args.model == 'pseudo_lidar':
                pc_lidar = dataset.get_pseudo_lidar(data_idx, 7)
                # pc_lidar = pc_lidar[:, 0:3]
            else:
                pc_lidar = dataset.get_lidar(data_idx)
...
if __name__ == '__main__':
    ...
    train_filename = output_prefix + '_train.pickle'
    val_filename = output_prefix + '_val.pickle'
    val_detections_filename = output_prefix + '_28_0037_rgb_detection.pickle'
    # rgb_detections = os.path.join(ROOT_DIR, 'data/KITTI/2d_detections/fpointnet/rgb_detection_val.txt')
    rgb_detections = "/data/zhaokexin/vision/3d_object/patchnet-master/test_yolo5lt_28_0037.txt"
    if args.gen_train:
        extract_frustum_data(split='train',
                             output_filename=train_filename,
                             perturb_box2d=True,
