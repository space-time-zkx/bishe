...
parser.add_argument('--small_only', action='store_true', help='Only generate frustum data from pedestrian and cyclist instances')
...
def random_shift_box2d(box2d, shift_ratio=0.05):
...
def extract_patch_data(split, output_filename,
    ...
    for data_idx in dataset.idx_list: # image idx
        ...
        # print(depth.shape,image.size)
        # assert depth.shape == image.size
        ...
        for object in objects:
            ...
            for _ in range(augment_times):
                ...
                # print(patch_xyz.shape)
                # patch_xyz = np.reshape(patch_xyz,(-1,3))
                # np.savetxt("patch.txt",patch_xyz)
...
def extract_patch_data_rgb_detection(det_filename, split, output_filename,
    ...
    for det_idx in range(len(det_id_list)):
        ...
        if cache_id != data_idx:
            ...
            # depth = depth.T
            # depth = np.array(depth).astype(np.float32) / 256
            # np.savetxt(str(det_idx) + "depth.txt", depth)
            # uvdepth = np.zeros((height, width, 3), dtype=np.float32)
            for v in range(height):
                for u in range(width):
                    uvdepth[v, u, 0] = u
...
if __name__ == '__main__':
    # setting
    whitelist = ['Pedestrian', 'Cyclist'] if args.small_only else ['Car', 'Pedestrian', 'Cyclist']
    output_prefix = 'patch_ped_cyc_only_' if args.small_only else 'patch_carpedcyc_vob'
    whitelist = ['Car'] if args.car_only else ['Car', 'Pedestrian', 'Cyclist']
    output_prefix = 'patch_car_only_' if args.car_only else 'patch_carpedcyc_vob'
    train_filename = output_prefix + 'train.pickle'
    val_filename = output_prefix + 'val.pickle'
    val_detections_filename = output_prefix + 'rgb_detection_val.pickle'
